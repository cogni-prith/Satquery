...
class ToolResult(BaseModel):
    ...
    answer_record: dict[str, Any] | None = Field(
        default=None,
        description=(
            "The serialised `AnswerRecord` the answer was worded from. Carried on the "
            "wire so a reader can check every claim against the fact that produced it: "
            "this is what makes the execution summary auditable rather than merely "
            "present. `None` for tools that do not go through the symbolic layer."
        ),
    )
